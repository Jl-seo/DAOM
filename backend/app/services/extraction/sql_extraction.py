...
async def _run_schema_mapper(markdown_text: str, model: ExtractionModel) -> Dict[str, Any]:
    ...
    - `"columns_mapping"`: Map EXPECTED TARGET KEYS (what the final schema wants, specified in `sub_fields`) to EXCEL COLUMN LETTERS ("A", "B", "C"...). 
      **SUPER CRITICAL MATCHING RULE: If you see a Markdown column header that closely matches the schema's `sub_field_key` (e.g., Markdown says 'POL' and Schema says 'pol', or Markdown says 'Origin MOT' and schema says 'motor'), YOU MUST MAP IT. Do NOT leave it out. Ignore case sensitivity and slight variations (spacing, symbols). If it explicitly looks like the field the user wants, MAP IT.** Do NOT use literal text headers here, ONLY the mapped column letter.
