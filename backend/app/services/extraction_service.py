...
class ExtractionService:
    ...
    async def run_extraction_pipeline(
        ...
        candidate_file_url: Optional[Union[str, List[str]]] = None, 
        candidate_file_urls: Optional[List[str]] = None, 
        candidate_filenames: Optional[List[str]] = None,
        **kwargs
    ):
        """
        Orchestrator: Downloads file -> Runs Extraction -> Updates Job
        Matches signatures from extraction_preview.py (positional), jobs.py (kwargs), and power_automate.py (positional list)
        """
        from app.services import extraction_jobs
        from app.services.storage import download_blob_to_bytes
        import mimetypes

        logger.info(f"[ExtractionService] Orchestrating job {job_id} for model {model_id}")

        try:
            # 1. Update Status
            extraction_jobs.update_job(job_id, status="analyzing")

            # 2. Normalize Inputs (Handle Polymorphism)
            primary_url = None
            
            # Case A: file_url passed as positional arg (str or list)
            if file_url:
                if isinstance(file_url, str):
                    primary_url = file_url
                elif isinstance(file_url, list) and len(file_url) > 0:
                    primary_url = file_url[0]
            
            # Case B: kwargs (jobs.py passes file_urls=...)
            if not primary_url and "file_urls" in kwargs:
                urls = kwargs["file_urls"]
                if urls and isinstance(urls, list) and len(urls) > 0:
                    primary_url = urls[0]
            
            if not primary_url:
                error_msg = "No file URL provided for extraction"
            ...
            try:
                ...
                logger.info(f"[ExtractionService] Downloaded {len(file_content)} bytes from {primary_url}")
            ...
            # 4. Handle Candidate Files (Comparison) - Placeholder
            # TODO: Implement comparison logic if candidate_file_urls present

            # 5. Run Core Extraction
            result = await self.extract_document(file_content, model_id, filename=filename, mime_type=mime_type or "")
            ...
            if "error" in result:
                ...
                logger.info(f"[ExtractionService] Job {job_id} completed successfully.")

        except Exception as e:
            logger.error(f"[ExtractionService] Fatal error in pipeline: {e}", exc_info=True)
            extraction_jobs.update_job(job_id, status="error", error=str(e))

    async def extract_document(self, file_content: bytes, model_id: str, filename: str = "", mime_type: str = "") -> Dict[str, Any]:
        """
